...
export async function getRoleKeyForUser(app: FastifyInstance, userId: number): Promise<AppRole | null> {
...
export async function getPermissionsForUser(app: FastifyInstance, userId: number): Promise<string[]> {
  const { rows } = await app.dbEngage.query(
    `
      select p.key
      from app_users u
