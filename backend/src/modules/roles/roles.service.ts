...
export async function updateRole(args: { app: FastifyInstance; roleId: number; key?: string; name?: string; description?: string | null }) {
  ...
  try {
    const { rows } = await args.app.dbEngage.query<RoleListItem>(
      `
      update app_roles
      set key = coalesce($1, key), name = coalesce($2, name), description = coalesce($3, description), updated_at = now()
      where id = $4
      returning id, key, name, description, is_system, created_at::text, updated_at::text,
      coalesce((select array_agg(p.key order by p.key)
        from app_role_permissions rp join app_permissions p on p.id = rp.permission_id
        where rp.role_id = app_roles.id), '{}'::text[]) as permissions
      `,
...
export async function replaceRolePermissions(args: { app: FastifyInstance; roleId: number; permissionIds: number[] }) {
  ...
  if (permissionRows.length !== args.permissionIds.length) throw Errors.badRequest("One or more permissions are invalid");
