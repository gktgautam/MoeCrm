...
export async function createUserInOrg(args: {
  ...
  const { rows: roleRows } = await args.app.dbEngage.query(`select id from app_roles where id = $1 limit 1`, [args.role_id]);
  if (!roleRows[0]) throw Errors.badRequest("Invalid role");
  ...
  try {
    const { rows } = await args.app.dbEngage.query<AppUserListItem>(
      ...
      returning id, email, (select key from app_roles where id = role_id) as role, status, created_at::text
      `,
      [args.email.toLowerCase(), args.role_id, args.status ?? "invited"],
    );
...
export async function updateUserInOrg(args: {
  ...
  const { rows } = await args.app.dbEngage.query<AppUserListItem>(
    `
      update app_users u
      set role_id = coalesce($1, role_id), status = coalesce($2, status), updated_at = now()
      from app_roles r
      where u.id = $3 and r.id = coalesce($1, u.role_id)
      returning u.id, u.email, r.key as role, u.status, u.created_at::text
    `,
    [args.role_id ?? null, args.status ?? null, args.targetUserId],
  );
